...
Packages <- c("tidyverse", "dbmss", "GoFKernel")
# Install them
...
On peut définir un diamètre équivalent $d_j^{eq}$ pour le voisin $j$ correspondant à un angle apparent $\alpha$ arbitrairement choisi, ce qui permet de caractériser chaque voisin par cette seule valeur telle que 
...
Pour un rayon $r$ quelconque autour de l'arbre $i$, $2 \pi r d_i^{eq}$ est la surface de la couronne de largeur $d_i^{eq}$.
...
la proportion de l'espace (unidimensionnel) encombré par les voisins ("Radial Crowding Ratio").
...
NCI <- function(i, ppp, r_max = 10) {
  ...
  return(
    ...
    sum(d_j_squared[is_not_center] / r_j[is_not_center])
  )
}
```
...
# Eliminate points close to edges
...
# Eliminate points close to edges
NCI_i_20 <- NCI_i_20[!is.na(NCI_i_20)]

# Equivalent d
d_i_eq_20 <- NCI_i_20 / 2 / pi
...
# Eliminate points close to edges
NCI_i_60 <- NCI_i_60[!is.na(NCI_i_60)]

# Equivalent d
d_i_eq_60 <- NCI_i_60 / 2 / pi
