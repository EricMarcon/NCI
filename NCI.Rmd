...
La fonction `NCI()` calcule les indices d'encombrement neutre et non neutre (voir plus loin).
...
## Corrélation avec NCI neutre

Les NCI prenant en compte la différence et la hierarchie des traits sont calculés pour WSG, log(SLA), et les classes de hauteur (de l'espèce, pas des arbres de la parcelle) et de taille de graine.

Les NCI hiérarchiques (pondérés par la différence des valeurs de traits standardisées) sont notés du nom du trait (ex.: "logSLA") et les NCI de différentiation de niche (pondérés par la valeur absolue de la différence) sont notés "abs_trait".
...
Le NCI neutre est très corrélé avec les 4 NCI de différentiation et avec les NCI hiérarchique liés à WSG et SLA. 
Les seuls NCI décorrélés sont ceux de la différentiation de niche liée à la hauteur et la masse des graines.

## Interprétation géométrique

L'interprétation est similaire à celle du NCI neutre.

On note $\delta_j$ la différence (éventuellement en valeur absolue) entre les valeurs de trait du voisin $j$ et de son arbre de référence.

Pour chaque voisin, $d_j^{eq} = \delta_j d_j \alpha_j / \alpha$.
En choisissant $\alpha = 2 \pi$, chaque voisin $j$ contribue à ${NCI}_i$ comme si son angle apparent était le cercle complet et que son diamètre était $d_j^{eq} = \delta_j d_j^2 / (2 \pi r_j)$.
